...
const imageMain = {
  backgroundImage: 'linear-gradient(to bottom, rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url(\'/assets/img/main.jpg\')',
  height: '100vh'
}
...
class HeaderComponent extends React.Component{
    ...
    componentWillMount(){
      request.get('https://tiendas-d6c6d.firebaseio.com/productos.json')
              .end((err, res)=>{
                if(err || !res.ok){
        ...
        (function loop(i, shoppingCart, productosFiltrados) {
            if (i < shoppingCart.length){
                new Promise(resolve => {
                    ...
                    request.patch(`https://tiendas-d6c6d.firebaseio.com/${ index }.json`,JSON.stringify(data)).end((err,res)=>{
